...
use screeps::{
    ...
    objects::Creep,
    StructureObject, Room, HasPosition, SharedCreepProperties, find, ObjectId, StructureSpawn, StructureController, Source, control, ErrorCode, Ruin, MaybeHasPosition, Position,
...
fn acquire_target(creep: &Creep, target_entry: hash_map::VacantEntry<'_, String, TargetByID>) -> Option<()> {
    let pos = creep.pos();
    ...
    else { match pos.find_closest_by_path(find::RUINS, None) {
        Some(r) => Some(TargetByObj::Ruin(r)),
